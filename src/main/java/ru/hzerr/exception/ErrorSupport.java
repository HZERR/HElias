...
public final class ErrorSupport {
    ...
    public static void showWarningPopup(String title, String message) {
        ...
        try {
            FXMLLoader.showPopup("warning", Properties.getInstance().getLanguage(), controller);
        } catch (IOException io) { showInternalError(io); io.printStackTrace();}
    }
    ...
    public static void showErrorPopup(Throwable throwable) {
        ...
        try {
            FXMLLoader.showPopup("error", Properties.getInstance().getLanguage(), controller);
